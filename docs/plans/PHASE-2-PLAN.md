...
- **A hand-written five-field cron parser** ([`internal/cron/cron.go`](../../internal/cron/cron.go)) with the day-of-month/day-of-week union rule already correct and tested. Report schedules use it. It has since moved out of `internal/maintenance` into a shared package, as this plan called for; it is not written twice.
