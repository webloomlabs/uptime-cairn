...
**Status: 2026-09-07. 131 of 133, and every feature box is ticked.** The two that
remain are the tagged release, which is a maintainer's decision rather than a
task, and the buffer, which is spent when that release goes out.

A report is defined in the dashboard, generated or fired by a schedule, rendered in
...
expires, and downloaded or previewed from a run-history screen that tells `partial`
from `succeeded` and a tombstone from a missing file, and its history browsed
over any past range at a resolution the screen names. Every claim below that is
ticked has a demonstrated run behind it, which is the rule this list has run
under since Phase 1 — with **two stated exceptions**, the preview and
historical-browsing screens, whose data paths were driven end to end but whose
controls nobody has clicked, because this repository has no browser-driver
tooling. The waiver note below says exactly what was and was not covered.

> ### The rules 1–2 and rule 7 waivers, 2026-09-07
>
> **Everything closed on 2026-09-07 was built by an agent under explicit
> maintainer waivers of [AGENTS.md](../../AGENTS.md).** Recorded here rather than
> in commit messages because this file is what somebody reads in a year.
>
> **Rules 1 and 2** ("No whole features", "No multi-file scaffolding runs") were
> waived for four vertical slices: the **certificate and domain expiry calendar**
> — the store filter, the `report.Store` contract, the document model, the
> composed block, the JSON artifact and the template editor; **preview**;
> **historical browsing**; and the **report-burst load phase** in the harness.
>
> **Rule 7** ("Never touch these without explicit human instruction: … CI
> configuration including the load-test gate") was waived for the gate itself.
> `-report-runs 50` in [`load-test.yml`](../../.github/workflows/load-test.yml)
> and the assertions behind it in `evaluateReports` are agent-written. That is
> the gate protecting the project's central promise, and it should be reviewed
> before it is trusted.
>
> **Rule 4 was not waived and was not needed anywhere.** The reporting contract
> was already frozen — `certificate_expiry` in the sections enum,
> `ReportDocument.expiries` typed, `UpcomingExpiry` schema'd, `/api/v1/expiries`
> shipped — and preview and browsing were built against endpoints that already
> existed, deliberately, rather than against new ones. The only additions are
> `MonitorIDs` on `ExpiryFilter` and the `Reporter` interface in the harness,
> both internal.
>
> **A tick on those boxes means the code works and has tests behind it. It does
> not mean a person has read it**, which is the same distinction the rule 8
> waiver draws below and is worth as much here. Two files deserve reading first:
> [`ArtifactPreview.svelte`](../../web/src/lib/components/ArtifactPreview.svelte),
> because it renders a document containing other people's words and its safety is
> entirely the sandbox attribute; and `evaluateReports` in
> [`scenario.go`](../../harness/scenario.go), because a gate nobody has checked is
> a gate that passes.
>
> **What the demonstrations could and could not cover.** The expiry calendar, the
> load phase and the two data paths behind the UI were driven end to end against
> a running binary and the numbers are quoted at their boxes. **The two UI screens
> were not clicked**: this repository has no browser-driver tooling, so what was
> demonstrated is the frontend building, the type-check passing, and the exact
> requests the new controls issue answering correctly — the artifact endpoint
> returning 4,183 bytes of script-free self-contained HTML for preview, and the
> history endpoint returning `1m`/`1h`/`1d` for an hour, thirty days and a year
> with truncated coverage detected on the year. That is short of a demonstrated
> run by this list's own standard, and it is said here rather than left to be
> assumed.
...
ruling, 2026-09-04): a missing file is rendered _as_ a missing file rather than
...
gate is **CI configuration under rule 7**, and it holds the worker-pool box open
with it — the pool is written and tested, and its exit criterion is the load test
saying so rather than the author. The remainder is the expiry calendar as a
_report type_ rather than as a collection, the historical drilldown screen, and
an HTML preview.
...
frozen spec: `deleteReportTemplate` is documented as _"Already-generated runs and
their artefacts are retained"_, and `report_runs.report_template_id` was `NOT
...
below used `SET NULL` because _"the artifact is a record of what a client was
sent, and it outlives the arrangement that sent it"_ — an argument about the
...
_of_ — which is the first question anybody asks of one, and the question that
...
`brand_profiles` is deliberately _not_ soft-deleted: a template referencing a
...
the document said _two breaches, fifty minutes_ beside _budget consumed: 0_.
...
in the log before it is stored: the path _is_ the credential, and the log gets
...
_notification channel_ has secret headers rather than a signing key — the HMAC
...
that channel's secret headers. A test asserts the _absence_ of a signature so the
gap stays visible.
...
gained the §3 tolerance sentence, which is the part that had to happen _now_:
...
backup documentation changes _in the same pull request_, because choosing files
...
the exit criterion is the procedure _tested end to end_, which is Month 7 work
...
- [x] `maintenance_handling` honoured as `exclude`, `count_as_up`, or `count_as_down`, with an empty value defaulting to `exclude` and _saying_ that it did. Whichever was used appears on the figure, not only in the template
...
- [x] Error budgets from `up_count` and `down_count` — [`sla.go`](../../internal/report/sla.go), eight tests. Target versus actual, budget consumed and remaining, burn rate. **No figure derives from a percentile.** Remaining goes negative rather than flooring at zero, because "41 minutes past" is the number somebody wants; a window that observed nothing consumes _nothing_ rather than everything, since a probe that never looked has not spent budget; and a 100% target omits the ratio and burn rate rather than returning an infinity that would render as `+Inf` on a client's PDF. A target met exactly is met — 99.9% arrives as 8991/9000, which is not exactly 99.9 in binary, and a naive comparison turns a met SLA into a breach
...
- [x] A bounded worker pool with an explicit concurrency limit, off the check path. **The pool is written and tested** — [`pool.go`](../../internal/report/runner/pool.go), two workers by default, a bounded queue that refuses rather than grows, and a `Submit` that does not block because the handler answering `202` must not wait for a worker. A full queue is a `503` naming the reason; an unbounded backlog would turn a bad morning into memory pressure and then into an OOM kill, taking the monitoring down with the reporting. **The exit criterion was the load test, and the load test now says so rather than the author.** [`MeasureReports`](../../harness/http.go) fires the burst and measures interference as a _pair_ — a quiet window, then a burst window — because interference cannot be read off one measurement: an install whose checks are late during a burst has either a reporting problem or a slow runner, and only the comparison says which. The measure is **check lateness rather than throughput**, since a pool that delayed every check by ten seconds and then caught up would show an unchanged rate over a sixty-second window and would have broken the promise exactly as stated. Measured at 1,000 monitors with 50 concurrent PDF renders: **lateness p95 1.00 → 1.00 intervals, writes 50/sec → 50/sec, 0 checks skipped, 0 results shed, 50 accepted and 50 completed in 27s**
...
- [x] **An opt-in live test against a real provider** — [`live_test.go`](../../internal/s3/live_test.go), skipped unless `CAIRN_S3_TEST_ENDPOINT` is set so that `go test ./...` on a laptop with nothing running stays green. It is what the vector test cannot be: a wrong secret is _refused_ by a server that actually checks it, which is the only way to tell a verified signature from an ignored one. Run against MinIO on 2026-09-03, including a key containing `+`, a space, `=` and `:`
...
- [x] **Certificate and domain expiry calendar** — built, and it turned out to be a **section rather than a type**, which is what the frozen contract already said: `type` is `[uptime, sla, post_mortem, comparative, custom]` and names no calendar, while `certificate_expiry` is in the sections enum and `ReportDocument.expiries` is a typed field of the frozen document. Nothing was invented; the shape was written and the code was missing. `report.Store` gains `ListUpcomingExpiries` **reused verbatim from the API's own store**, the way `ListIncidents` already is — one collection behind `/api/v1/expiries` and the report block, so the screen an operator checks and the document a client is sent cannot come to disagree about what is expiring. Scope narrowing needed a `MonitorIDs` on `ExpiryFilter`: a report's scope has already resolved to monitor IDs by the time it reaches the store, and it **intersects** with the tag filter rather than unioning, because too much in a client's document is a disclosure where too little is a gap somebody notices. Demonstrated against the running binary with two clients' monitors on one install, five expiry rows between them: an Acme report drew the lapsed `acme.example` (`expired 12 days ago`, leading the table) and the 11-day `acme-api.example`, while **the rival client's two expiries — the soonest on the install at 1 and 2 days — did not appear**, and neither did Acme's own registration renewed five years out. The same run produced the block in the HTML and the PDF from one composition, and an `sla` template naming the same section returned `expiries: []` and drew nothing
- [x] **The calendar is read for `custom` reports and no others**, the same gate the incident log runs under and the same reason: it is another read, and the four-reads-whatever-the-scope property the extended load gate measures is worth keeping for the types that make up almost every scheduled report. No other type can name the section that would draw the block, so nothing pays for a block it cannot render. An empty scope does not ask at all — which matters beyond the saved read, because an empty `MonitorIDs` means _every monitor_ to the store, and the query would have answered the emptiest possible document with the whole install's calendar
- [x] **`days_remaining` counts from the run's own `generated_at`, not the wall clock.** ADR-007 requires the same model rendered twice to be byte-identical, and a figure counted from `time.Now` breaks that inside a single run: the PDF and the JSON of one report, rendered either side of midnight, would disagree by a day about how long a certificate has left. The horizon is **90 days**, fixed rather than configurable — the widest of the three §4.8 names, and the one that makes the other two readable off the same table, since a reader wanting thirty days reads the top of a ninety-day list where the reverse does not work. Unbounded was the alternative and is worse: a domain registered for ten years would head the table with an expiry nobody acts on this decade. It is not on the frozen template contract and inventing a field for it would be an API change
- [x] **Custom builder** — `sections` now chooses which blocks the composition emits, which is the half that made `custom` a builder rather than a synonym for "everything". [`sections.go`](../../internal/report/render/sections.go) resolves a template's selection into a layout and [`Compose`](../../internal/report/render/compose.go) walks it. **Order is honoured rather than treated as a set**, because the frozen spec says "ordered content blocks" and a set would have quietly narrowed the contract — with one structural rule that is the only non-literal part: five of the ten sections are per-monitor, so that group is emitted **once, at the position of the first per-monitor section named**, and within each monitor its blocks appear in the order they were named. Scattering them would repeat the monitor's heading per block and turn a three-monitor report into fifteen headings. A monitor with none of its blocks selected is **not introduced at all**, because a page of monitor names with nothing under each reads as a rendering fault rather than as a choice. `sla_breakdown` and `error_budget` draw one block between them: the spec separates them and ADR-006 does not, since the budget is computed from the same counts as the target-versus-actual line and rendered beside it. Demonstrated against the running binary — the same template with `["sla_breakdown","response_time"]` and `["response_time","sla_breakdown"]` produced the two headings in the two orders, and naming both SLA sections drew the block once

**Three things this pass settled that were not on the list.** `sections` was stored and round-tripped with **no validation at all** — harmless while nothing read it, and a silent missing block the moment it did, since the composer drops what it cannot recognise rather than failing a queued run. It is now validated at the API with a pointer naming the offending element and a message listing the vocabulary. **The cover, the methodology note and the footer are deliberately not deselectable**: the note carries the denominator and the maintenance policy, and §4.3 makes both an obligation of the report face — a figure whose policy can be switched off cannot be checked by the person holding it — and the frozen enum names none of the three, so nothing the API offers is being refused. And **selection reaches the HTML and PDF and deliberately not the JSON or CSV**: those are the data document and a row per bucket, not a page, and a data export whose columns depended on a presentation choice would be worse than one that ignores it.

- [x] `maintenance_log` is in the frozen enum, accepted by the API, and composes no block — stated in `Compose` rather than left to be discovered, because a maintenance log needs a windows query `report.Store` does not have. The template editor does not offer it, because a chip that silently contributes nothing is a control that appears broken. **`certificate_expiry` has left this box** and is offered — but only on a `custom` template, since that is the only type whose document carries a calendar, and the chip is hidden rather than disabled on the others: a disabled control invites the reader to work out what would enable it, and the answer is a decision about the whole document rather than about one block. A template switched away from `custom` drops the selection on save rather than keeping a block nobody can see to remove
...
- [x] **The schedules screen** — list, create, edit and delete at `/reports/schedules`, and a **Schedule** link beside **Generate** on each template row, because those are the two things somebody looking at a template is choosing between. **This was missing entirely and was not on this list**, which is the part worth recording: [PHASE-2-PLAN.md §2](PHASE-2-PLAN.md) names the UI deliverables as "list, definition editor, run history, preview, download, and the custom builder" and never names a schedule editor, so the checklist had been ticking off a UI section specified without one. Automatic delivery to a client is the headline of the phase and it was the one part of it a person could not reach — every schedule had to be created by hand-writing JSON at `/api/v1/report-schedules`. The list leads on **next run**, because a schedule is configuration nobody looks at again until something has gone wrong and the question people arrive with is always "is this going to fire, and when"; a paused schedule still shows its firing time, greyed, since "it would have gone out on the 1st and it is switched off" is the sentence somebody needs and a blank cell reads as broken rather than paused. Delivery targets are repeatable rows that disclose only the fields their type uses, and the S3 row says on its face that it is the **drop and not the mirror**

**Building it found a defect that had been unreachable from the API alone: a cron schedule could never become a monthly one.** `cron` is a `*string`, so `null` and omitted are the same nil — the stale expression carried forward, `CronFor` refused the combination, and the schedule was unsaveable for as long as it existed. The only way out was an empty string, which nothing documents and no client would guess. It is now cleared when the frequency moves off cron, **without softening the refusal beside it**: a body that still asserts an expression alongside a non-cron frequency is refused exactly as before, because a stored expression that never runs is a schedule an operator believes they configured. Four tests hold the four cases apart. This is the second defect this phase that was found by driving a screen rather than by reading the handler.

- [x] **Preview**, beside Download on a rendered HTML artifact. It reads the existing artifact endpoint rather than a preview endpoint, because the frozen spec declares none and inventing one would be rule 4: `Content-Disposition: attachment` governs top-level navigation and says nothing about `fetch`, so the same URL the download link points at serves the preview and the two cannot disagree about what the report says. **The stored artifact, never a re-render** — the same rule the share link follows, and for the same reason: a preview that re-rendered would show figures that had moved since the client got theirs, which defeats the one question people bring to it. Only the HTML artifact is offered: a PDF would need a viewer the dashboard does not have, and the CSV and JSON are data exports that read worse in a panel than in a file. **The frame carries a bare `sandbox` with no tokens at all** — a unique opaque origin, no script, no access to the session — because the document carries a brand's own words and monitor names, subjects and issuers that came off the wire from a probed certificate; under the sandbox an escaping bug in the renderer stays a rendering bug instead of becoming a session compromise. **Security-relevant and agent-written: [`ArtifactPreview.svelte`](../../web/src/lib/components/ArtifactPreview.svelte) is the file to read first**
...
- [x] The custom builder in the UI — a **Content** picker on the template editor, offering the eight sections that compose something, showing each ticked block's **position** because order is part of what the control sets and a checkbox alone would hide that the second thing ticked is the second thing rendered, and saying plainly that the JSON and CSV ignore it. Nothing ticked reads as "the standard blocks for this type" rather than as an empty report
...
- [x] **Full historical browsing with drilldown into arbitrary past ranges, always labelled with the resolution actually used.** 90-day and 1-year presets join the four that were there, and a **Custom range** control takes explicit start and end times — a preset answers "how has it been lately" and only an absolute range answers "what happened in March", which is the question a relative picker cannot express at all. The data layer needed nothing: `/monitors/{id}/history` already took `from`, `to` and `resolution` and already returned the tier that answered.

  **The labelling is the part that makes it honest, and it took three sentences rather than one.** Every chart states the resolution it was drawn at, because the tier is chosen from the span and a reader comparing a year of daily buckets against last week's hourly ones without being told is comparing different measurements. Where a range reaches back further than the install holds, the chart says **when the data actually begins** — derived by comparing the first bucket against the requested start, with a bucket's width of slack, since this endpoint carries no `covered_from` the way `meta.resolution` does and adding one would be an API change. And **an empty range no longer looks like an untroubled one**: drilling into a month past the hourly tier's retention asks for hours that no longer exist and draws nothing while the daily buckets for that same month are still there, so the screen says the range holds nothing _at this resolution_ rather than leaving the chart's "no heartbeats" to read as a fact about the monitor
...
- [x] **The two documented failure modes, confirmed rather than assumed.** Restoring without the key refuses to start with the exact message the guide prints and **does not generate a replacement** — the failure that would otherwise leave every stored credential unreadable while appearing to work. And restoring `cairn.db` _without_ `reports/` leaves an install that starts, lists every run, and answers the download honestly
...
- [x] **The 5,000-monitor gate extended: 50 concurrent report runs with check scheduling latency unchanged.** A regression blocks merge, not release. `-report-runs 50` is wired into the engine job in [`load-test.yml`](../../.github/workflows/load-test.yml), running at the largest scale only — fifty renders against a 500-monitor install measures the pool against an engine with nothing much to delay, which would pass whatever the pool did. **This is CI configuration and was written under an explicit rule 7 waiver (see the head of this file); it needs reviewing as such.** The assertions are on deltas rather than absolutes, which is what lets the gate run on a shared runner at all, and the tolerances are named constants with their reasoning at their sites: a quarter of an interval of added lateness, ten per cent of write rate. A refusal from the bounded queue is **reported without failing** — answering `503` is the design — while refusing _everything_ fails, because a queue that accepts nothing is too small to be useful
...
- [x] Crash-consistency — [`crash_test.go`](../../internal/report/runner/crash_test.go). The _state_ a kill at that instant leaves is reproduced rather than the kill itself: a test that forked and SIGKILLed a child would be testing the harness, and the only observable difference between the two orderings is the file present with the row absent. Three assertions, each a different failure — the file at the final path is **complete**, because the rename makes a partial one impossible there; **no row refers to it**, which is what makes it an orphan rather than a dangling row; and the sweeper reclaims it once past the grace period, so the residue is bounded rather than permanent. A second test states the harm the ordering avoids as a checkable claim rather than a comment: a dangling row is **invisible to the sweeper**, because walking the disk cannot find a missing file — a different fault with a different answer, and one that deleting nothing can fix

## Release

- [ ] Tagged release, images, checksums, SBOM, changelog, announcement — the automation exists and has now run, so this is a repeat rather than a first. **Prepared but deliberately not run.** The changelog entry is written, [the release checklist](../releases/1.1.0-checklist.md) is the sequence with the irreversible step marked, and [the announcement](../releases/1.1.0-announcement.md) is a draft needing a human's judgement about tone. The tag itself is a maintainer's decision and stays one: pushing it publishes images that cannot be replaced once pulled, only superseded, and rule 6 forbids an unattended release. **1.1.0 is also the tag that freezes `/api/v1` for the reporting operations**, so the checklist puts reading the agent waivers and deciding ADR-009 before the tag rather than after
- [ ] Buffer. **Features get cut before the release date or the quality gates do.** Not a deliverable and not tickable on its own: it is the slack the phase carries, and it is spent when the release goes out. Nothing was cut — every feature box above is ticked and the gates were extended rather than relaxed
