...
**Status: 2026-09-03. 99 of 125, and the reporting subsystem is end to end.** A
report is defined in the dashboard, generated or fired by a schedule, rendered in
four formats, filed on disk with a digest, delivered by email or webhook,
reclaimed when it expires, and downloaded from a run-history screen that tells
`partial` from `succeeded` and a tombstone from a missing file. Every claim below
that is ticked has a demonstrated run behind it, which is the rule this list has
run under since Phase 1.

**What is left is four things and they are named honestly.** Share links and the
S3 client are **human-led under [AGENTS.md](../../AGENTS.md) rule 8** — an
unauthenticated token and a request signer are not an agent's to generate — and
the contract test names the four share operations in its skip list with that
reason, so deleting those four lines is the last step of building them. The
extended load gate is **CI configuration under rule 7**. The backup drill is
Month 7 work that needs artifacts to drill against, and it now has them. The
remainder is smaller: `sections` selecting blocks in `Compose` (which is the
custom builder), the expiry calendar as a *report type* rather than as a
collection, and the historical drilldown screen.
...
- [ ] Download is built — a link per rendered artifact, with the failed and expired ones labelled rather than offered. **Preview and the share link are not**: preview needs a viewer for an HTML artifact and share links are human-led work under rule 8, so the control that would create one has nothing to call
