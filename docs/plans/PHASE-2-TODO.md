...
**Status: 2026-09-04. 124 of 131, and the reporting subsystem is end to end.** A
...
- [x] `maintenance_log` and `certificate_expiry` are in the frozen enum, accepted by the API, and compose no block — stated in `Compose` rather than left to be discovered. A maintenance log needs a windows query `report.Store` does not have; the certificate table is the expiry-calendar report type, which is the open box below. The template editor does not offer either, because a chip that silently contributes nothing is a control that appears broken
